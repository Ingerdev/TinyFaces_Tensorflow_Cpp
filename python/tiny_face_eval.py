...
def save_model_as_tf(model,model_dir,model_filename):

  saver = tf.train.Saver()
  with tf.Session() as sess:
    sess.run([tf.global_variables_initializer()])
    tf.train.write_graph(sess.graph.as_graph_def(), model_dir, "mymodel.pb")
...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
  ...
  if (False):
    save_model_as_tf(score_final,'C:/Projects/Home/Ingerdev/Commercial/FindActor/TestPad/TestTinyfaceTensorflow/','mymodel.ckpt')
    return;
  ...
  model.dump_channel_averages(average_image)
  model.dump_clusters(clusters)
  model.close_serializer()
  # main
  ...
  with tf.Session() as sess:
    ...
    for filename in filenames:
      ...
      def _calc_scales():
        ...
        max_w = np.max(clusters_w[normal_idx] / raw_w);
        max_h = np.max(clusters_h[normal_idx] / raw_h);
      ...
      for s in scales:
        ...
        #sess.run(tf.train.export_meta_graph(filename='my-model.meta'))
        #print("saved")
        #break;
        ...
        def _calc_bounding_boxes():
          ...
          ft, fy, fx, fc = np.where(prob_cls_tf > prob_thresh)

          #check uniquiness
          #match = list(map(''.join,list(zip(ft.astype(str),fx.astype(str),fy.astype(str),fc.astype(str)))))
          #from collections import Counter
          #print([k for k, v in Counter(match).items() if v > 1])
      ...
      # save image with bounding boxesclient_session.h
